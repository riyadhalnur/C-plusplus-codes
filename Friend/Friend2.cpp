...
public:
	friend int Match( Course C, Room &R );
	Room( int n );
};
...
int Match( Course C, Room &R )
{
	if( R.TimeAlloc[C.T] == '0' && R.DayAlloc[C.D] == '0' ){
		R.Assign( C.T, C.D );
		cout << "Room " << R.Number << " Assigned to Course " << C.Name << " at " << Times[C.T] << " on " << DayName[C.D] << endl;
		return 1;
	}
	cout << "Room " << R.Number << " Occupied at " << Times[C.T] << " on " << DayName[C.D] << endl;	
	return 0;
}
